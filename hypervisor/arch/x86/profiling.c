...
static int profiling_generate_data(int32_t collector, uint32_t type)
{
	...
	if (collector == COLLECT_PROFILE_DATA) {
		...
		pr_err("%s: unknown collector specified %d",
			__func__, collector);
	}
	return 0;
}
...
static int profiling_pmi_handler(__unused int irq, __unused void *data)
{
	...
	/* Save the vmm context info */
	get_cpu_var(sep_info.vmm_ctx).rip = exec_vmread64(VMX_GUEST_RFLAGS);
	get_cpu_var(sep_info.vmm_ctx).rflags = exec_vmread64(VMX_GUEST_RIP);
	get_cpu_var(sep_info.vmm_ctx).cs = exec_vmread64(VMX_GUEST_CS_SEL);
...
int profiling_msr_ops_all_cpus(const struct vm *vm, uint64_t addr)
{
	...
	struct profiling_msr_ops_list *msr_list
			= (struct profiling_msr_ops_list *)gpa2hva(vm, addr);
 ...
 */
int profiling_vm_info_list(const struct vm *vm1, uint64_t addr)
{
	...
	if (copy_from_gpa(vm1, &vm_info_list, addr, sizeof(vm_info_list)) != 0) {
	...
	if (copy_to_gpa(vm1, &vm_info_list, addr, sizeof(vm_info_list)) != 0) {
...
int profiling_config_pmi(const struct vm *vm, uint64_t addr)
{
	...
	struct profiling_pmi_config *pmi_config
			= (struct profiling_pmi_config *)gpa2hva(vm, addr);;
...
void profiling_setup(void)
{
	...
	get_cpu_var(sep_info.sep_state).vmexit_msr_cnt = 0;
